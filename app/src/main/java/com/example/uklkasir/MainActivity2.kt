...
class MainActivity2 : AppCompatActivity() {
    ...
    private var listInstantly = mutableListOf<Menu>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recyclerInstantly = findViewById(R.id.recyclerInstantly)
        ...
        adapterInstantly = ItemAdapter(listInstantly)
        adapterInstantly.onAddClick = {
            listCart.add(it.id_menu)
            checkoutButton.isEnabled = true
            checkoutButton.visibility = View.VISIBLE
            checkoutButton.text = "Checkout (" + listCart.size + ")"
        ...
        adapterInstantly.onSubstractClick = {
            listCart.remove(it.id_menu)
            if(listCart.size == 0){
                checkoutButton.isEnabled = false
                checkoutButton.visibility = View.INVISIBLE
        ...
        recyclerInstantly.adapter = adapterInstantly
        recyclerInstantly.layoutManager = LinearLayoutManager(this)


        swipeToGesture(recyclerMakanan)
        swipeToGesture(recyclerMinuman)
        swipeToGesture(recyclerSnack)
        swipeToGesture(recyclerInstantly)
